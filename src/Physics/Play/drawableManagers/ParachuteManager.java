...
public class ParachuteManager {
   ...
   /* public void addParachutesToRobots(List<Parachute> parachutes, List<Robot> robots) {
        for(int i = 0; i < parachutes.size(); i++)
        {
            ...
            robots.get(i).addDrawable(parachutes.get(i));
            parachutes.get(i).addDrawable(robots.get(i));
        }
    ...
    public void move(List<Parachute> parachutes) {
        ...
        for (int i = 0; i < parachutes.size(); i++)
        {
            parachutes.get(i).setY(parachutes.get(i).getDrawable(Robot.class).getY() - (parachutes.get(i).getHeight() - 20));
        }
    }
    ...
    public void checkForRemoval(List<Parachute> parachutes) {
        for(int i = 0; i < parachutes.size(); i++)
            if(parachutes.get(i).isActive() == false)
                parachutes.remove(i);
    }
}
